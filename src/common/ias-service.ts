...
/**添加选人分组 */
function saveMemberGroup(groupName, refTableIds) {
    return request<any>(
        `${config.unpcApiHost}memberGroup/save`,
        {
            method: 'POST',
            data: {
                groupName,
 ...
 * @returns
 */
function delMemberGroup(groupName) {
    return request<any>(`${config.unpcApiHost}memberGroup/del`, {
        method: 'POST',
        data: {
            groupName
        }
